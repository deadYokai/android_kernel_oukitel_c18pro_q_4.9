...
static SENSOR_FUNCTION_STRUCT_1 *g_pSensorFunc_1[1] = {&SensorFuncGC032AYUV_1};
...
#endif
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (g_invokeSocketIdx[i] == DUAL_CAMERA_MAIN_SENSOR) {
				ret =
					g_pInvokeSensorFunc[i]->SensorGetInfo(
						(enum
...
{
	MUINT32 ret = ERROR_NONE;
	MUINT32 buf_sz1 = sizeof(struct ACDK_SENSOR_EXPOSURE_WINDOW_STRUCT);
	MUINT32 buf_sz2 = sizeof(struct ACDK_SENSOR_CONFIG_STRUCT);
...
int kdSetDriver(unsigned int *pDrvIndex)
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		...
		g_invokeSocketIdx[i] = (enum CAMERA_DUAL_CAMERA_SENSOR_ENUM)
...
#ifdef MTK_SUB2_IMGSENSOR
	struct ACDK_SENSOR_RESOLUTION_INFO_STRUCT *pRes[3] = {
	NULL,
	NULL,
	NULL};
#else
	...
	case SENSOR_FEATURE_SET_AE_WINDOW:
		{
			...
			pApWindows =
				kmalloc(sizeof(MUINT32) * 6, GFP_KERNEL);
			if (pApWindows == NULL) {
				kfree(pFeaturePara);
				PK_ERR(" ioctl allocate mem failed\n");
				return -ENOMEM;
	...
	case SENSOR_FEATURE_GET_DELAY_INFO:
		{
			...
			pDelayInfo =
				kmalloc(sizeof(struct SENSOR_DELAY_INFO_STRUCT),
					GFP_KERNEL);
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
		{
			...
			pPdaf_data =
				kmalloc(sizeof(char) * PDAF_DATA_SIZE,
				GFP_KERNEL);
			if (pPdaf_data == NULL) {
				kfree(pFeaturePara);
				PK_ERR(" ioctl allocate mem failed\n");
...
static int __init CAMERA_HW_i2C_init(void)
{
	...
	proc_create("driver/pdaf_type", 0000, NULL, &fcamera_proc_fops_set_pdaf_type);
