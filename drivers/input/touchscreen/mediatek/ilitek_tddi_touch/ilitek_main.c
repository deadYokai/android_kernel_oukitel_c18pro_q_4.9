...
//+ add by songbinbo.wt for High sensitivity mode contrl after suspend resume 20191125
static void ilitek_glove_work_queue(struct work_struct *work)
{
...
#if WT_ADD_TP_HARDWARE_INFO
char * ven_name[] = {"TRULY_ROW","TRULY_NA","TXD_ROW","TXD_NA","TRULY_TCL","NONE"};
...
static void ilitek_resume_by_ddi_work(struct work_struct *work)
{
	...
	if (idev->fw_upgrade_mode == UPGRADE_IRAM){
            if(ilitek_tddi_fw_upgrade_handler(NULL)<0){
                ipio_err("FW upgrade failed when resume by ddi work.\n");
            }
        }else{
	...
	idev->tp_suspend = false; //add by songbinbo.wt for ilitek double tap to wakeup optimize the power consumption 20191209
	mutex_unlock(&idev->touch_mutex);
}
...
#endif
	INIT_WORK(&idev->glove_work_queue, ilitek_glove_work_queue);//+ add by songbinbo.wt for High sensitivity mode contrl after suspend resume 20191125
}
...
/*ritchie add first irq*/
/*
	if(idev->first_irq){
		ipio_info("ignore int triggered by first irq\n");
		idev->first_irq = false;
		return;
	}
*/
	if ((idev->irq_after_recovery)||(idev->first_irq)) {
		ipio_info("ignore int triggered by recovery or by first irq.\n");
