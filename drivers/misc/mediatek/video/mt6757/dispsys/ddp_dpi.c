...
static void lcm_udelay(unsigned us) { udelay(us); }

static void lcm_mdelay(unsigned ms) { msleep(ms); }
...
/*clksrc = DPI_CK;*/
	...
	if (!clk_apmixed_base)
		...
		DPI_OUTREG32(NULL, clk_apmixed_base + 0x270,
			     con0); /* TVDPLL enable */
		DPI_OUTREG32(NULL, clk_apmixed_base + 0x274,
			     prediv); /* set TVDPLL output clock frequency */
	}
...
enum DPI_STATUS ddp_dpi_ConfigVsync(struct cmdqRecStruct *cmdq,
				    enum DPI_POLARITY polarity,
				    unsigned pulseWidth, unsigned backPorch,
				    unsigned frontPorch)
{
...
enum DPI_STATUS ddp_dpi_ConfigDualEdge(struct cmdqRecStruct *cmdq, bool enable,
				       unsigned mode)
{
	struct DPI_REG_OUTPUT_SETTING ctrl = DPI_REG->OUTPUT_SETTING;
...
enum DPI_STATUS ddp_dpi_ConfigSize(struct cmdqRecStruct *cmdq, unsigned width,
				   unsigned height)
{
...
int ddp_dpi_power_on(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	pr_debug("DISP/DPI, %s, s_isDpiPowerOn %d\n", __func__,
		 s_isDpiPowerOn);
	if (!s_isDpiPowerOn) {
		   ...
		   struct disp_ddp_path_config *config, void *cmdq_handle)
{
	if (s_isDpiConfig == FALSE) {
		...
		pr_debug("DISP/DPI, %s DPI status:%x, cmdq:%p\n", __func__,
		...
		pr_debug("DISP/DPI,dpi_config->bg_width: %d, "
...
int ddp_dpi_init(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	DPI_OUTREGBIT(cmdq, struct DPI_REG_INTERRUPT,
		      DPI_REG->INT_ENABLE, VSYNC, enInt.VSYNC);

	enInt.VDE = 1;
	DPI_OUTREGBIT(cmdq, struct DPI_REG_INTERRUPT,
		      DPI_REG->INT_ENABLE, VDE, enInt.VDE);

	enInt.UNDERFLOW = 1;
	DPI_OUTREGBIT(cmdq, struct DPI_REG_INTERRUPT,
		      DPI_REG->INT_ENABLE, UNDERFLOW, enInt.UNDERFLOW);
#endif
#endif
#if 0
#if !defined(CONFIG_MTK_LEGACY)
